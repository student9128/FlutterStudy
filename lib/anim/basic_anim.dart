...
class _BasicAnimState extends State<BasicAnim> with TickerProviderStateMixin {
  ...
  AnimationController? _aCurveController;
  late Animation<double> _aCurve;
  double value = 0;
  ...
  void initState() {
    ...
    _animationRotate =
        Tween(begin: 0.0, end: 1.0 * pi).animate(_animationController!);
    _animationOffset = Tween(begin: Offset(0, 0), end: Offset(0.7, .3))
    ...
    //开始动画
    // _animationController?.forward();
    _aCurveController = AnimationController(
        duration: Duration(milliseconds: 2000), vsync: this);
    _aCurve = Tween(begin: 0.0, end: 2000.0).animate(_aCurveController!)
      ..addListener(() {
        value = _aCurveController!.value;
        setState(() {});
      })..addStatusListener((status) {
        if(status==AnimationStatus.completed){
          // _aCurveController?.reverse();
        }
      });
  }

  Widget rectangleContainer = Container(
    width: 50,
    height: 50,
    color: Colors.red,
  );
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          ElevatedButton(
              onPressed: () {
                debugPrint('status==${_animationController!.status}');
                if(_aCurveController!.status==AnimationStatus.completed){
                  _aCurveController!.reverse();
                }else{
                  _aCurveController!.forward();
                }
              },
              child: Text('绘制曲线动画')),
          Container(
            ...
            height: 300,
            child: CustomPaint(
              painter: ChartLine(animValue: value),
            ),
          )
        ],
      ),
